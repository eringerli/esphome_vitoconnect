...
namespace esphome {
  namespace vitoconnect {
    ...
    void VitoConnect::setup() {
      ...
      ESP_LOGD(
        TAG, "Starting optolink with protocol: %s", this->protocol.c_str() );
    ...
    void VitoConnect::register_datapoint( Datapoint* datapoint ) {
      ESP_LOGD( TAG,
                "Adding datapoint with address %x and length %d",
                datapoint->getAddress(),
                datapoint->getLength() );
    ...
    void VitoConnect::update() {
      ...
      for( Datapoint* dp: this->_datapoints ) {
        ...
        if( _optolink->read( dp->getAddress(),
                             dp->getLength(),
